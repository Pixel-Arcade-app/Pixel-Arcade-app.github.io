...
(()=>{
  ...
  async function signup(email,pw,d){
    email=String(email||'').trim();const pseudo=String(d.pseudo||'').trim(),first=String(d.first_name||'').trim(),last=String(d.last_name||'').trim(),phone=String(d.phone||'').trim(),age=Number(d.age);
    ...
    if(!first)throw Error('Le prénom est obligatoire.');if(!last)throw Error('Le nom de famille est obligatoire.');
    ...
    let res;try{res=await api('/auth/v1/signup',{method:'POST',body:JSON.stringify({email,password:pw,data})})}catch(e){if(/already|exists|registered|email_exists|user_already_exists/i.test(String(e.message)))throw Error('Cette adresse e-mail est déjà utilisée.');throw e}
    ...
    const s=state();s.user={access_token:res.access_token,refresh_token:res.refresh_token,user:res.user};s.online=true;if(window.PAAuth.refresh)await window.PAAuth.refresh();return {logged:true,pseudo};
  }
