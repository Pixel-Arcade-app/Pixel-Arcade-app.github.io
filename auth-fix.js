...
(()=>{
  ...
  async function signup(email,pw,d){
    ...
    let res;
    try{
      res=await api('/auth/v1/signup',{method:'POST',body:JSON.stringify({email,password:pw,data})});
    }catch(e){
      if(/already|exists|registered|email_exists|user_already_exists|already registered/i.test(String(e.message)))throw Error('Cette adresse e-mail est déjà utilisée.');
      throw e;
    }
    // Supabase peut volontairement répondre 200 avec un utilisateur sans identité
    // lorsqu'une adresse existe déjà. On transforme ce cas en message clair.
    ...
    const s=state();
    s.user={access_token:res.access_token,refresh_token:res.refresh_token,user:res.user};
    s.online=true;
    if(window.PAAuth.refresh)await window.PAAuth.refresh();
  ...
  function modal(){
    ...
    function form(tab){
      ...
      m.querySelector('#paSubmit').onclick=async()=>{
        const msg=m.querySelector('#paAuthMsg'),b=m.querySelector('#paSubmit');
        msg.textContent='';b.disabled=true;
        try{
          let r;
          if(tab==='signup'){
            const ids=['paEmail','paPseudo','paFirstName','paLastName','paAge','paPass'];
            if(!ids.every(id=>m.querySelector('#'+id).reportValidity()))throw Error('Vérifie les champs obligatoires.');
            if(!m.querySelector('#paTerms').checked)throw Error('Tu dois accepter les conditions d’utilisation.');
            r=await signup(m.querySelector('#paEmail').value,m.querySelector('#paPass').value,{pseudo:m.querySelector('#paPseudo').value,first_name:m.querySelector('#paFirstName').value,last_name:m.querySelector('#paLastName').value,age:m.querySelector('#paAge').value,phone:m.querySelector('#paPhone').value});
          }else{
            r=await window.PAAuth.emailLogin(m.querySelector('#paIdentifier').value,m.querySelector('#paPass').value);
          }
          msg.textContent=r.message||((r.pseudo||'Compte')+' connecté !');
          window.dispatchEvent(new Event('pa-auth-updated'));
          if(r.logged)setTimeout(()=>m.remove(),700);
        }catch(e){
          const raw=String(e?.message||'');
          if(/invalid login credentials|connexion impossible|identifiants invalides|vérifie.*identifiants|user not found/i.test(raw))msg.textContent='Erreur : pseudo/e-mail ou mot de passe incorrect.';
          else msg.textContent='Erreur : '+raw;
