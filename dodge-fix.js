(()=>{
  ...
  const ready=()=>{
    ...
    const style=document.createElement('style');style.id='dodgeMobileFix';style.textContent=`
      ...
      .wrap>.result{position:absolute;inset:0;z-index:8;margin:0;display:none;align-items:center;justify-content:center;border-radius:16px;background:rgba(2,6,23,.88);backdrop-filter:blur(5px);padding:22px;text-align:center}
      ...
      .wrap>.result h2{margin:0;font-size:clamp(1.35rem,3vw,2rem)}
      .wrap>.result p{margin:5px 0;line-height:1.5}
      .wrap>.result input{background:#0c1424;color:#fff;border:1px solid #475569}
      .wrap>.result .result-actions{justify-content:center}
      body.light .wrap>.result{background:rgba(255,255,255,.9);color:#172033;border-color:#d5deea}
      ...
      @media(max-width:560px){.wrap>.result{padding:12px}.wrap>.result h2{font-size:1.35rem}.wrap>.result p{font-size:.9rem}.wrap>.result input{margin:5px 0}}
