(()=>{if(window.__PA_DODGE_V3_ACTIVE__)return;window.__PA_DODGE_V3_ACTIVE__=true;
  ...
  function start(){if(running)return;reset();pointerTarget=null;running=true;startAt=performance.now();last=startAt;pausedTotal=0;$('startOverlay').classList.add('hidden');$('result').classList.remove('show');$('result').style.display='none';unlock('first');raf=requestAnimationFrame(loop)}
